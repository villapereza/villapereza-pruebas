(() => {
  ...
  async function showBackendVersion() {
    ...
    const webVersion = (window.VP_CONFIG && window.VP_CONFIG.APP_VERSION) || '8.0.2';
    try {
  ...
  function renderUsers() {
    ...
    const requestsByUser = groupBy(state.data.requests, 'userId');
    ...
    els.usersTableBody.innerHTML = state.data.users.map(user => {
      ...
      const requestedTests = (requestsByUser[user.id] || [])
        .map(item => testsById[item.testId])
        .filter(test => test && test.active)
        .sort((a, b) => dayIndex(a.day) - dayIndex(b.day) || a.order - b.order || sortByName(a, b));
      ...
      const requestTags = requestedTests.length
        ? requestedTests.map(test => `<span class="user-request-tag"><small>${VP.escapeHtml(dayShort(test.day))}</small>${VP.escapeHtml(test.name)}</span>`).join('')
        : `<span class="no-user-tests">${isParticipant ? 'Sin solicitudes' : 'No aplica'}</span>`;
      ...
      <tr class="${user.active ? '' : 'row-inactive'}">
        ...
        <td><div class="user-tests-cell">${requestTags}</div></td>
